...
data_path = '/Users/ali/Desktop/Mar23/amd_mixed/bundle_analysis/new_method_4-13/15/Statistics_bundlesizelim_distance_15_affinerigid_non_inclusive_symmetric/bundle_stats_withid/'
...
bundle_match_path = '/Users/ali/Desktop/Mar23/amd_mixed/bundle_analysis/new_method_4-13/15/Statistics_bundlesizelim_distance_15_affinerigid_non_inclusive_symmetric/'
...
msr_data_ctrl$new_bundle = msr_data_ctrl$Bundle.ID
msr_data_amd$new_bundle = NA 
for (i in ID_matcher[,3]) {
  #print(i)
  print(ID_matcher[,2][which(ID_matcher[,3] == i)])
  msr_data_amd$new_bundle [  msr_data_amd$Bundle.ID ==i ] = ID_matcher[,2][which(ID_matcher[,3] == i)]
}
msr_data_matching_bdl = rbind(msr_data_amd, msr_data_ctrl)
msr_data_matching_bdl = filter(msr_data_matching_bdl,msr_data_matching_bdl$new_bundle %in% c(0,1,2,3,4,5)    )


lm <- lmer( fa~AMD+ (1|Subject) + new_bundle  + Point.ID   ,data=msr_data_matching_bdl, REML = TRUE)
