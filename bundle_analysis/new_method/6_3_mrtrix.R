options(java.parameters = "-Xmx2048m")  

library(xlsx)
library(lme4)
library(lmerTest)
library(multcomp)
library(rstatix)


data_path = '/Users/ali/Desktop/Mar23/amd_mixed/bundle_analysis/new_method/Statistics_allregions_streamsvsallsubj_distance_6_affinerigid_non_inclusive_symmetric/bundle_stats_withid/'

msr_data_amd  = read.csv( paste0(data_path,'2Year_AMD_ctx-lh-lingual_left_to_right-cerebellum-cortex_right_all_bundle_stats.csv' ) )
msr_data_amd$AMD = 1 


#fa
subjects_forbiden  = unique(msr_data_amd$Subject)
colnames = colnames(msr_data_amd)
index_all = grep ( "fa", colnames(msr_data_amd))
index_forbiden = grep(paste0("fa",".",subjects_forbiden,collapse="|"), colnames(msr_data_amd))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_amd[, index]
msr_data_amd$fa = rowMeans(temp)

#md
subjects_forbiden  = unique(msr_data_amd$Subject)
colnames = colnames(msr_data_amd)
index_all = grep ( "md", colnames(msr_data_amd))
index_forbiden = grep(paste0("fa",".",subjects_forbiden,collapse="|"), colnames(msr_data_amd))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_amd[, index]
msr_data_amd$md = rowMeans(temp)

#rd
subjects_forbiden  = unique(msr_data_amd$Subject)
colnames = colnames(msr_data_amd)
index_all = grep ( "rd", colnames(msr_data_amd))
index_forbiden = grep(paste0("fa",".",subjects_forbiden,collapse="|"), colnames(msr_data_amd))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_amd[, index]
msr_data_amd$rd = rowMeans(temp)


#ad
subjects_forbiden  = unique(msr_data_amd$Subject)
colnames = colnames(msr_data_amd)
index_all = grep ( "rd", colnames(msr_data_amd))
index_forbiden = grep(paste0("ad",".",subjects_forbiden,collapse="|"), colnames(msr_data_amd))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_amd[, index]
msr_data_amd$ad = rowMeans(temp)



msr_data_ctrl  = read.csv( paste0(data_path,'2Year_Control_ctx-lh-lingual_left_to_right-cerebellum-cortex_right_all_bundle_stats.csv' ) )
msr_data_ctrl$AMD = 0



#fa
subjects_forbiden  = unique(msr_data_ctrl$Subject)
colnames = colnames(msr_data_ctrl)
index_all = grep ( "fa", colnames(msr_data_ctrl))
index_forbiden = grep(paste0("fa",".",subjects_forbiden,collapse="|"), colnames(msr_data_ctrl))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_ctrl[, index]
msr_data_ctrl$fa = rowMeans(temp)

#md
subjects_forbiden  = unique(msr_data_ctrl$Subject)
colnames = colnames(msr_data_ctrl)
index_all = grep ( "md", colnames(msr_data_ctrl))
index_forbiden = grep(paste0("fa",".",subjects_forbiden,collapse="|"), colnames(msr_data_ctrl))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_ctrl[, index]
msr_data_ctrl$md = rowMeans(temp)

#rd
subjects_forbiden  = unique(msr_data_ctrl$Subject)
colnames = colnames(msr_data_ctrl)
index_all = grep ( "rd", colnames(msr_data_ctrl))
index_forbiden = grep(paste0("fa",".",subjects_forbiden,collapse="|"), colnames(msr_data_ctrl))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_ctrl[, index]
msr_data_ctrl$rd = rowMeans(temp)


#ad
subjects_forbiden  = unique(msr_data_ctrl$Subject)
colnames = colnames(msr_data_ctrl)
index_all = grep ( "rd", colnames(msr_data_ctrl))
index_forbiden = grep(paste0("ad",".",subjects_forbiden,collapse="|"), colnames(msr_data_ctrl))
index = setdiff(index_all,index_forbiden)
#colnames[index]
temp =  msr_data_ctrl[, index]
msr_data_ctrl$ad = rowMeans(temp)








# table_amd = table(msr_data_amd$Subject)/50
# table_amd = as.data.frame(t(table_amd))
# table_amd$AMD = 1
# table_ctrl = table(msr_data_ctrl$Subject)/50
# table_ctrl = as.data.frame(t(table_ctrl))
# table_ctrl$AMD = 0
# table = rbind(table_amd,table_ctrl )
# 
# anova(lm( Freq~AMD     ,data=table, REML = TRUE))
# 
# 
# 





msr_data = rbind(msr_data_amd, msr_data_ctrl)

bundle_match_path = '/Users/ali/Desktop/Mar23/amd_mixed/bundle_analysis/new_method/Statistics_allregions_streamsvsallsubj_distance_6_affinerigid_non_inclusive_symmetric/'
ID_matcher = read.csv( paste0(bundle_match_path,'2Year_ctx-lh-lingual_left_to_right-cerebellum-cortex_right_all_bundle_order.csv' ) )

bdl_num =1
amd_bdl = ID_matcher[bdl_num,]$Paired.2.YR.AMD
ctrl_bdl = ID_matcher[bdl_num,]$Paired.2.YR.Control

# amd_bdl = ID_matcher[bdl_num,]$Paired.Initial.AMD
# ctrl_bdl = ID_matcher[bdl_num,]$Paired.Initial.Control


msr_data_amd_bdl = msr_data_amd[ msr_data_amd $Bundle.ID == amd_bdl, ]
msr_data_ctrl_bdl = msr_data_ctrl[ msr_data_ctrl $Bundle.ID == ctrl_bdl, ]
# 
# table_amd = table(msr_data_amd_bdl$Subject)/50
# table_amd = as.data.frame(t(table_amd))
# table_amd$AMD = 1
# table_ctrl = table(msr_data_ctrl_bdl$Subject)/50
# table_ctrl = as.data.frame(t(table_ctrl))
# table_ctrl$AMD = 0
# table = rbind(table_amd,table_ctrl )
# 
# anova(lm( Freq~AMD     ,data=table, REML = TRUE))


msr_data_bdl = rbind(msr_data_amd_bdl, msr_data_ctrl_bdl)
dim(msr_data_bdl)/50
table(msr_data_bdl$Subject )/50
#removing based on number of streamline
# table = as.data.frame(table(msr_data_bdl$Subject )/50)
# index_left_1s_qtr = table$Freq > quantile(table$Freq, 0.25)
# left_1s_qtr_subj = table$Var1[index_left_1s_qtr]
# msr_data_bdl = msr_data_bdl[msr_data_bdl$Subject %in% left_1s_qtr_subj,]
####



lm <- lmer( md~AMD+ (1|Subject)  + Point.ID   ,data=msr_data, REML = TRUE)

a=anova(lm)
a
a$`Pr(>F)`
#c=summary(glht(lm_winding_trials, emm(pairwise ~ agecat|Stage)))
d=eta_squared(lm)
e=effectsize::cohens_f(lm, alternative='two.sided')



