...
bundle_match_path = '/Users/ali/Desktop/may23/amd_mixed/new_cond_minmm_strmlines/Statistics_bundlesizelim_distance_15_MDT_non_inclusive_symmetric_mrtrix/'

ROIS_names = c('ctx-rh-lingual_right_to_ctx-lh-lingual_left', 'ctx-lh-lingual_left_to_right-cerebellum-cortex_right', 'ctx-rh-lingual_right_to_left-cerebellum-cortex_left')
#ROIS_names = c('ctx-rh-lingual_right_to_ctx-lh-lingual_left')
...
if (lencon ==FALSE &  max(cohen)> 0 )
{ cohenposind= which(cohen>0)    
  if (min(pval_adj_fdr[cohenposind]) <0.05) {     
    
    semi_final_winners = which(cohen >0 & pval_adj_fdr<0.05  )
    semi_index = which(pval_adj_fdr[semi_final_winners] == min(pval_adj_fdr[semi_final_winners]) )
    winner_ind = semi_final_winners[semi_index][1]
  }
else{
...
mypath = '/Users/ali/Desktop/may23/amd_mixed/new_cond_minmm_strmlines/'
