...
for (i in 1:dim(pvals)[1]) {
  ...
  if (!is.na(pval_adj_bnf[i]) ){
    if ( pval_adj_bnf[i]<=0.05 ) cat("Bundle Top" , i   , "th is bnfrni significant with pval", pval_adj_bnf[i] , " \n" )
  }
  if (!is.na(pval_adj_fdr[i]) ){
    if ( pval_adj_fdr[i]<=0.05 ) cat("Bundle Top" , i   , "th is fdr significant with pval",  pval_adj_fdr[i]  , " \n" )
